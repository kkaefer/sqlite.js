    ...
    this._retrievePage(no, Page.BTREE, function(err, page) {
        ...
        if (page.leaf) {
            ...
            async.map(pages, db._loadPageData.bind(db), function(err, data) {
                if (err) return callback.call(db, err);
                callback.call(db, null, data.reduce(function(a, b) { return a.concat(b); }))
            });
        }
    });
}
        ...
        page.getCell(no).getRecord(function(err, fields, cell) {
            ...
            for (var i = 0; i < args.length; i++) {
                if (fields[i] > args[i]) {
                    ...
                    return callback(null, cell.left);
                } else if (fields[i] < args[i]) {
                    if (no + 1 >= page.cells) {
                        // Continue with right page.
                        return callback(null, page.right);
                    } else {
            ...
            // All fields match.
            return callback(null, false, fields);
        });
    }
...
Database.prototype._btreeSearch = function(no, args, callback) {
    ...
    this._retrievePage(no, Page.BTREE, function(err, page) {
        ...
        function next(err, next, result) {
            if (err) return callback(err);
            if (result) return callback(null, result);
            if (next) return db._btreeSearch(next, args, callback);
        ...
        if (page.type & Page.INDEX) {
            db._scanIndexPage(page, args, next);
        } else {
            db._scanTablePage(page, args, next);
        }
    });
};
