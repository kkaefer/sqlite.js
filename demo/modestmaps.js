    ...
    MM.Layer.prototype = {
        ...
        inventoryVisibleTile: function(layer_element, tile_coord) {
            ...
            if (!this.requestManager.hasRequest(tile_key)) {
                if (this.provider.getTile.length == 3) {
                    var _addTileElement = MM.bind(this.addTileElement, this);
                    this.provider.getTile(tile_coord, tile_key, _addTileElement);
                } else {
            ...
            for (var pz = 1; pz <= maxStepsOut; pz++) {
                ...
                if (this.enablePyramidLoading) {
                    ...
                    if (parent_key in this.tiles) {
                        ...
                        if (this.provider.getTile.length == 3) {
                            var _addTileElement = MM.bind(this.addTileElement, this);
                            this.provider.getTile(parent_coord, parent_key, _addTileElement);
                        } else {
                            var tileToAdd = this.provider.getTile(parent_coord);
                            if (typeof tileToAdd == 'string') {
                                this.addTileImage(tile_key, tile_coord, tileToAdd);
                            // tile must be truish
                            } else if (tileToAdd) {
                                this.addTileElement(tile_key, tile_coord, tileToAdd);
                            }
                        }
                    }
                } else {
